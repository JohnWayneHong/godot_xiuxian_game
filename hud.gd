...
func _ready() -> void:
	...
	_animate_button($SingleButton, Color(1.15, 1.15, 1.05), 1.12, 0.90)
	_animate_button($DuoButton, Color(1.2, 1.1, 0.9), 1.12, 0.90)
	_animate_button($SettingsButton, Color(1.1, 1.2, 1.2), 1.08, 0.92)
	_animate_button($LeaderboardButton, Color(1.1, 1.2, 1.3), 1.08, 0.92)
	_animate_button($GameOverPanel/VBoxContainer/SubmitButton, Color(1.2, 1.1, 0.9), 1.08, 0.92)
	_animate_button($GameOverPanel/VBoxContainer/ButtonRow/MenuButton, Color(1.25, 1.25, 1.25), 1.06, 0.94)
	_animate_button($GameOverPanel/VBoxContainer/ButtonRow/ViewLeaderboardButton, Color(1.1, 1.2, 1.3), 1.06, 0.94)
	_animate_button($LeaderboardPanel/VBoxContainer/BackButton, Color(1.25, 1.25, 1.25), 1.06, 0.94)
	_animate_button($SettingsPanel/VBoxContainer/SettingsBackButton, Color(1.25, 1.25, 1.25), 1.06, 0.94)
...
func _show_character_select(for_player: int, mode: StringName = &"") -> void:
	_selecting_for_player = for_player
	_hide_character_select()  # 清理旧面板
	_char_cards.clear()
...
func _hide_character_select() -> void:
	...
	# NOTE: 不重置 _selecting_for_player 和 _pending_mode，由各自调用方管理状态
...
func _show_main_menu() -> void:
	...
	# 重置分数显示
	$ScoreLabel.text = "0"
	# 清除游戏结束时设置的主题颜色覆盖，让彩虹效果接管
	$Message.remove_theme_color_override("font_color")
	$MessageGlow.remove_theme_color_override("font_color")
	_rainbow_active = true
	$RainbowTimer.start()
	$Message.text = "Dodge the Creeps!"
	$MessageGlow.text = "Dodge the Creeps!"
	$Message.show()
	$MessageGlow.show()
	$SingleButton.show()
	$DuoButton.show()
